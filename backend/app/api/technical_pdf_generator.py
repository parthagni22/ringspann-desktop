...
def generate_backstop_technical_pdf(quotation_number, metadata, requirements, technical_quotes, filepath):
    ...
    for idx, req in enumerate(requirements):
        ...
        row = [
            ...
            Paragraph(get_field_value(cust_reqs, 'Tag Number', 'tagNumber', 'tag_number'), cell_style),
            Paragraph(get_field_value(cust_reqs, 'Application', 'application'), cell_style),
            Paragraph(get_field_value(cust_reqs, 'Shaft diameter (d)', 'shaftDiameter', 'shaft_diameter'), cell_style),
            Paragraph(get_field_value(cust_reqs, 'Torque (Mn) Min', 'torqueMin', 'torque_min'), cell_style),
            Paragraph(get_field_value(cust_reqs, 'Torque (Mn) Max', 'torqueMax', 'torque_max'), cell_style),
            Paragraph(get_field_value(cust_reqs, 'Speed Min (RPM)', 'speedMin', 'speed_min'), cell_style),
            Paragraph(get_field_value(cust_reqs, 'Speed Rated', 'speedRated', 'speed_rated'), cell_style),
            Paragraph(get_field_value(cust_reqs, 'Speed Max', 'speedMax', 'speed_max'), cell_style),
...
def generate_locking_element_technical_pdf(quotation_number, metadata, requirements, technical_quotes, filepath):
    """Generate Locking Element for Conveyor Technical Quotation PDF - EXACT TEMPLATE MATCH"""
    ...
    cell_style = ParagraphStyle('CellStyle', fontSize=3.8, alignment=TA_CENTER, leading=4.5)
    header_style = ParagraphStyle('HeaderStyle', fontSize=3.8, alignment=TA_CENTER, leading=4.5, fontName='Helvetica-Bold')
    
    # LOCKING ELEMENT EXACT COLUMNS FROM TEMPLATE (32 columns!)
    # Four-row header structure with complex spanning
    ...
    for idx, req in enumerate(requirements):
        ...
        row = [
            ...
            Paragraph(get_field_value(cust_reqs, 'Running Tension tight side belt', 'runningTensionBeltTight', 'running_tension_belt_tight'), cell_style),
            Paragraph(get_field_value(cust_reqs, 'Running Tension slack side belt', 'runningTensionBeltSlack', 'running_tension_belt_slack'), cell_style),
            ...
            Paragraph(get_field_value(cust_reqs, 'Starting Tension tight side belt', 'startingTensionBeltTight', 'starting_tension_belt_tight'), cell_style),
            Paragraph(get_field_value(cust_reqs, 'Starting Tension slack side belt', 'startingTensionBeltSlack', 'starting_tension_belt_slack'), cell_style),
            # Arm, wrap, factors
